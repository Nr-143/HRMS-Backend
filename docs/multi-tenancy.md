  ...
  query: {
    $allModels: {
      async $allOperations({ model, operation, args, query }) {
        ...
        // 1. If there's no active user session (e.g. signup, login, health check), bypass the filter
        if (!context || !context.tenantId) {
          return query(args);
        }

        // 2. If the user is a platform SUPER_ADMIN, bypass the filter to query across all tenants
